...
export async function callChatLLM(
  ...
  const request: LLMRequest = {
    ...
    messages: messages,
    abortSignal: signal,
    llm_call_annotation: `Message ${messageId} callChatLLM`,
  };
  let streamText = "";
  let thinkText = "";
  let toolArgsText = "";
    ...
    while (true) {
      ...
      switch (chunk.type) {
        ...
        case "finish": {
          ...
          const dialogueUsage = (chunk.usage || {}) as Record<string, any>;
          const dialogueProviderMeta = ((chunk as any).providerMetadata?.anthropic || {}) as Record<string, any>;
          const dialogueCachedInput = dialogueUsage.cachedInputTokens || dialogueUsage.cache_read_input_tokens || dialogueProviderMeta.cacheReadInputTokens || 0;
          const dialogueCacheCreation = dialogueUsage.cacheCreationInputTokens || dialogueUsage.cache_creation_input_tokens || dialogueProviderMeta.cacheCreationInputTokens || 0;
          await streamCallback.onMessage({
            type: "finish",
            finishReason: chunk.finishReason,
            usage: {
              promptTokens: dialogueUsage.inputTokens || 0,
              completionTokens: dialogueUsage.outputTokens || 0,
              totalTokens:
                dialogueUsage.totalTokens ||
                (dialogueUsage.inputTokens || 0) +
                  (dialogueUsage.outputTokens || 0),
              cachedInputTokens: dialogueCachedInput || undefined,
              cacheCreationInputTokens: dialogueCacheCreation || undefined,
            },
          });
          break;
        }
