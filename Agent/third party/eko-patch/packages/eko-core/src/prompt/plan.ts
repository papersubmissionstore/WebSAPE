...
<root>
  ...
  <agents>
    ...
    <agent name="Browser" id="1" dependsOn="0">
      ...
      <nodes>
        <node>Open WeChat web version</node>
...
<thought>The user wants me to research information about 'Fellou', create a summary profile, and share it on multiple social media platforms (Twitter, Facebook, Reddit). This requires multiple Browser agents working together: one for research and parallel ones for social media posting (Twitter, Facebook, and Reddit). I need to break this down into sequential steps with proper variable passing between agents.</thought>
<agents>
  <agent name="Browser" id="0" dependsOn="">
    ...
    <agent name="Browser" id="4" dependsOn="1,2,3">
      <task>Compile and summarize social media campaign results</task>
      <nodes>
        <node input="twitterResults,facebookResults,redditResults">Summarize all social media posting results including Platform, Post URL, Content Summary, Timestamp, and Initial Engagement</node>
      </nodes>
    </agent>
  </agents>
</agents>
